...
#define SOC_IO_MODE_NONBLOCK	(0)

//#define CERT_FROM_NVM
...
int ssl_demo_wait_network(void)
{
    if(ol_wait_network_regist(120) != mbtk_data_call_ok)
    {
        op_uart_printf("ssl_demo_wait_network ol_wait_network_regist time out\n");
        return -1;
    }
    return 0;
}
...
int ssl_socket_demo(void)
{
    ...
    ret = ssl_demo_wait_network();
    if(ret != 0)
    {
        op_uart_printf("ssl_socket_demo ssl_socket_demo_wait_network fail\n");
        return -1;
    }
    ...
    ssl_cfg.serverPort = 30063;
    ssl_cfg.verify = SSL_VERIFY_MODE_NONE;
    ssl_cfg.vsn = SSL_VSN_ALL;
#if 0//def CERT_FROM_NVM
...
#if SOC_IO_MODE_NONBLOCK
		int noneblock = 1;
		ol_timeval tv = {0};
		
		ol_ssl_set_optation(SSL_OP_BLOCK,&noneblock);
		tv.tv_sec = 5;
		ol_ssl_set_optation(SSL_OP_TIMEOUT,&tv);
#endif
    ol_ssl_set_config(&ssl_ctx,&ssl_cfg);

    ret = ol_ssl_ctx_init(&ssl_ctx);
    op_uart_printf("SSLCtxInit ret=%d",ret);

	if(ret == 0)
	{
		ret = ol_ssl_handshake(&ssl_ctx,60*1000);
    	op_uart_printf("SSLHandshake ret=%d",ret);

		if(ret == 0)
		{

			ret = ol_ssl_write(&ssl_ctx,"hello server",12);
		   	op_uart_printf("SSLWrite ret=%d",ret);
			if(ret > 0)
#if SOC_IO_MODE_NONBLOCK
			{
				char read_count = 5;
				while(read_count)
				{
		   		...
		   		op_uart_printf("SSLRead ret=%d",ret);
		   		op_uart_printf("SSLRead recv data=%s",buff);
					read_count--;
				}	
			}
#else
		    ...
		    op_uart_printf("SSLShutdown ret=%d",ret);
		}
